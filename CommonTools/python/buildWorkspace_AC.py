...
parser.add_option("--path",dest="path",help="path to the file you want to read")
...
#basepath = '%s/src/CombinedEWKAnalysis/CommonTools/data/anomalousCoupling_WGG'%os.environ['CMSSW_BASE']
...
# check if shape uncertainty name is one of those where Signal and bkg uncertainties are correlated
    ...
    print '$$$$$$$$$$$$$ \t $$$$$$$$$  setting up for %s plane!'%signalModel,' limitType: ', limtype, ' Ndim: ',Ndim
    ...
    print '%s/signal_proc_%s.root'%(basepath,codename)
    if (Ndim==1):
