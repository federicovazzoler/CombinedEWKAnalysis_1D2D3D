...
for section in fit_sections:
  ...
  outfile_newF = TFile.Open(options.output+'/data/post_fit/FIT_'+coupling_name.split("_")[0]+'_'+section+'.root','RECREATE')
...
#        func.FixParameter(i,func.GetParameter(i)/SM_yield)
  ...
  f = open(options.output+'/cards/range_'+coupling_name.split("_")[0]+'.txt', 'w')
